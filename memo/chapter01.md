...
また インスタンスも従来`Hello hello = new Hello()`の hello のことを指しているけど 僕は変数という認識だった
go でインスタンスは 型の値 つまり変数という認識で僕は違和感が無い
...
#### テスト駆動開発に基づいたパッケージ作成
現状 ブラウザを複数開いて動作確認する方法のテストしかない
チャットアプリのユニットテストは作らないがログの記録するパッケージを作る
ログとは 処理の流れを表示したり記録するもの
`log.Println()`を使って出力していたものを 定式的に行えるパッケージを作る
今回は　TDD に従って作成する

go でのパッケージは 1つのディレクトリが1つのパッケージになる
1つのディレクトリに置かれたファイルはすべて同じパッケージに属すると考えられる
go にはサブパッケージという概念がない
ログのパッケージは 呼び出されることはあっても直接実行されることはないため main じゃないパッケージにするべき
すると 拡張性が高く柔軟なパッケージになるようモデル化して API を考える必要がある
フィールド, 関数, メソッド, 型 どれを公開して非公開にするか考える
大文字で始まる名前は公開(エクスポート)されて 小文字で始まる名前は非公開(プライベート)になる

インタフェースから作成しているが 訳者いわくインタフェースありきのAPIが必ず良いとは言えない
でも僕もインタフェースは定義だと思うから 今は作りたい派かな
いろいろ触れたらまた変わるかも

`...interface{}`は 任意の型の引数を0個以上受け取ることを表す
`fmt.Sprint`などでもこの書き方がされていて 複数の物事をまとめて表現できるようになる利点がある

go は生まれた当初から テストのツールは含まれていた
go でテストするための仕様で
- テストコードはテスト対象と同じディレクトリに置く
- ファイルの末尾は`_test.go`
- テストコードのファイルは`import testing`する
- 関数の名前は`Test`で始まる
- テストする関数は`*testing.T`型の引数を1つ受け取る
これら条件を満たす関数はテストコードのあるディレクトリまで移動して`go test`とするとテストが実行される

io.Writer であれば何でも受け付ける
io.Writer インタフェースで定義されている Write メソッドを実装していれば良い
つまり ユーザーが出力先を選べるということ
例えば 標準出力, ファイル, ネットワークのソケット, *byte.Buffer, 自作オブジェクト など
これらは Write メソッドを持っている

go にはカバレッジを算出するツールも標準で備わっている
`go test -cover`で割合が出力される
カバレッジは高いほど良い

Tracer はインタフェースだから オブジェクト(struct)を返すように変更する
そのときに インタフェースで定義したメソッドを持っていないといけないから 実装する

Trace() で渡された引数を struct に格納して テストが通るようになった

os.stdout を使用するとターミナルに出力される

自作パッケージの trace は2点問題がある
コンソールに出力するだけのログは要らないこと
room 型に Tracer がセットされていないと落ちる
解決するために trace.Off メソッドを実装する

nilTracer を実装するとこでコンソールにログ出力しないようにできた
room 型に Tracer がセットされていないと落ちる問題は room struct を生成するときにデフォルトで　nilTracer をセットする
ターミナル等で表示させたいときは`r.tracer = trace.New(os.Stdout)`で出力先を指定して使う

Go 言語ではソースコード中にコメントを書くことで自動的に API ドキュメントを生成する方法が最初からサポートされている
コメントは type, variable, constant, function, package に書くことができる
宣言の直前(空白も空けずに)にコメントで書く
ただし その要素名でコメントを書き始めるルールがある
__ただ使い方は go のバージョン変化とともに色々あるようでよく分からなかった__

#### まとめ
net/http パッケージを使うと　並列のWebサーバが立てれる
WebSocket を実装することもできた
template を使ってコードとコンテンツを分離できた
template にデータを埋め込む方法を知った
チャネルを使って 同期が必要な箇所を明示して 複数オブジェクトからの同時アクセスに対応した
go での TDD を学んだ
...
>`once.Do(f)` が複数回呼び出された場合 f の値が呼び出しごとに異なっていても、最初の呼び出しのみがfを呼び出します。
>各関数を実行するには、Onceの新しいインスタンスが必要です。
